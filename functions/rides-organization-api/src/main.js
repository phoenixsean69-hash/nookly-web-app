...
const requiredConfig = () => {
  if (!DATABASE_ID || !ORGANIZATIONS_COLLECTION_ID) {
    throw statusError(
      500,
      "The organization driver-review function is missing database configuration.",
    );
  }
};
...
export default async ({ req, res, log, error }) => {
  try {
    ...
    if (
      ...
      return ok(
        res,
        await buildApplication(tablesDB, organization.$id, parts[1]),
      );
    }
    ...
    if (
      ...
      log?.(
        `Organization ${organization.$id} approved driver ${parts[1]}.`,
      );
